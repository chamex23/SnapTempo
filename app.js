...
navigator.geolocation.getCurrentPosition(
  ...
  (err) => {
    if (err.code === 1) {
      alert(
        "Geolocalização não permitida pelo usuário. Busque manualmente na barra de pesquisa."
      );
    } else {
      console.log(err);
    }
...
function displayWeather(data) {
  ...
  currentTemperature.textContent = `${Math.round(temp)}Cº`;
  windSpeed.textContent = `${Math.round(speed * 3.6)}km`;
  feelsLikeTemperature.textContent = `${Math.round(feels_like)}Cº`;
...
function formatDate(epochTime) {
  ...
  let formatteDate = date.toLocaleDateString("pt-br", {
    month: "long",
    day: "numeric",
  });
